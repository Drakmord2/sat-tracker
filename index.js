...
function gridToLatLon(grid) {
    ...
    let lon = (lonField * 20) + (lonSquare * 2) + (lonSub * 5.0 / 60) + (2.5 / 60);
    let lat = (latField * 10) + latSquare + (latSub * 2.5 / 60) + (1.25 / 60);
    ...
    function renderDropdown(filteredSatellites) {
        ...
        filteredSatellites.forEach(satellite => {
            ...
            div.addEventListener('click', function () {
                searchInput.value = satellite.name;
...
function formatGroupedPassesToHTML(groupedPasses) {
    ...
    groupedPasses.forEach((pass, index) => {
        ...
        const highestTime = pass.highest?.time ? new Date(pass.highest.time).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : "未知时间";
        ...
        const exitTime = pass.exit?.time ? new Date(pass.exit.time).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : "未知时间";
        ...
        const eventTitle = `${satelliteName}  ${highestElevation}`;
...
function formatGroupedPassesToHTMLfavorite(allGroupedPasses) {
        ...
        <table border="1" style="border-collapse: collapse; width: 100%; text-align: center;">
            <thead>
                <tr>
                    ...
                    <th rowspan="2">${translations[currentLang].tableHeaders.date}</th>
                    <th rowspan="2">${translations[currentLang].tableHeaders.start}</th>
                    <th rowspan="2">${translations[currentLang].tableHeaders.satellite}</th>
                    <th colspan="2">${translations[currentLang].tableHeaders.highest}</th>
                    <th rowspan="2">${translations[currentLang].tableHeaders.end}</th>
                </tr>
                <tr>
    ...
    allPasses.forEach((item, index) => {
        ...
        const entrydate = pass.entry?.time ? new Date(pass.entry.time).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit' }) : "未知时间";
        const entryTime = pass.entry?.time ? new Date(pass.entry.time).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit' }) : "未知时间";
        ...
        const highestTime = pass.highest?.time ? new Date(pass.highest.time).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : "未知时间";
        const highestElevation = typeof pass.highest?.elevation === "number" ? `${pass.highest.elevation.toFixed(2)}°` : "未知";
        const exitTime = pass.exit?.time ? new Date(pass.exit.time).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit' }) : "未知时间";
